...
let authFile = `${opts._[0] || 'login'}.json`
...
global.reloadHandler = function () {
  ...
  conn.welcome = 'Halo, @user!\nSelamat Datang Di Grup @subject\n\n@desc\n Follow Instagram-Ku Donk: @_yusakarif'
  conn.bye = 'Selamat Tinggal @user\nSemoga Kamu Gak Lupa Sama Temen Kamu Yang Ada Di Sini Ya'
  conn.spromote = '@user Telah Diangkat Menjadi Seorang Admin Di Group Ini'
  conn.sdemote = '@user Telah Diturunkan Menjadi Member Biasa Di Group Ini'
...
async function _quickTest() {
  ...
  if (!s.ffmpeg) conn.logger.warn('Silahkan Install Terlebih Dahulu FFMPEG Agar Tidak Ada Kesalahan / Kerusakan Pada Sticker,Gambar, Ataupun Video')
  if (s.ffmpeg && !s.ffmpegWebp) conn.logger.warn('Mohon Perhatiannya, Apakah LIBWEBP Sudah Terinstall? Karena Tanpa LIBWEBP Sticker Tidak Dapat Beranimasi')
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('Silahkan Install Terlebih Dahulu IMAGEMAGICK Agar Tidak Ada Kecacatan Saat Mengkonversi Sticker/Gambar/Video Ataupun Media Lainnya')
}

_quickTest()
  .then(() => conn.logger.info('Quick Test Selesai'))
  .catch(console.error)

//Memberi Info Kepada Owner Apabila Bot Telah Online
conn.sendMessage('6289671522780@c.us', 'Bot Is Online Now!', 'conversation');