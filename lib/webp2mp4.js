...
async function gif2mp4(source) {
  ...
  form.append('new-image-url', isUrl ? source : '')
  form.append('new-image', isUrl ? '' : source, 'image.webp')
  let res = await fetch('https://s6.ezgif.com/gif-to-mp4', {
    method: 'POST',
    body: form
  })
  let html = await res.text()
  ...
  let html2 = await res2.text()
  let { document: document2 } = new JSDOM(html2).window
  return new URL(document2.querySelector('div#output > p.outfile > mp4 > source').src, res2.url).toString()
}
...
gif2mp4('https://media1.tenor.com/images/4c742132b3f7018e49ae1901a2ab3e68/tenor.gif?itemid=5027512').then(console.error)
} else {
  module.exports = { webp2mp4, webp2png, gif2mp4 }
}